...
def get_gaze_direction_normalized(iris_center, eye_landmarks):
    """Calcula las coordenadas normalizadas del iris dentro del ojo."""
    # Calcula el rectángulo delimitador del ojo
...
cv2.namedWindow('Deteccion de Distraccion (MediaPipe)')
...
with mp.solutions.face_mesh.FaceMesh(
    ...
    while True:
        ...
        if not ret:
            print("ERROR: Fallo al leer un frame de la cámara.")
            break
        ...
        if face_detected_by_dnn and results.multi_face_landmarks:
            ...
            cv2.polylines(frame, [np.int32(left_eye_points)], True, (0, 255, 0), 1, cv2.LINE_AA)
            cv2.polylines(frame, [np.int32(right_eye_points)], True, (0, 255, 0), 1, cv2.LINE_AA)
            ...
            if left_iris_center is not None and len(left_eye_points) > 0 and right_iris_center is not None and len(right_eye_points) > 0:
                ...
                print(f"Left Eye - Normalized X: {normalized_x_left:.2f}, Normalized Y: {normalized_y_left:.2f}  |  Right Eye - Normalized X: {normalized_x_right:.2f}, Normalized Y: {normalized_y_right:.2f}")
            ...
            if distracted_frames >= FOCUS_THRESHOLD:
